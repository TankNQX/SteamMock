...
| `gen/steam_interfaces.json` | The interface layouts the stub hands out: names, version strings, slot order and argument kinds. **Not in this repository** - it is Valve's data, and whoever builds it imports theirs from an SDK with `tools/steamworks_sdk_import.py`. Validated by the generator - see [Interface layouts](#interface-layouts). |
...
imports them from an SDK they have - several generations at a time, oldest first - with
...
same as from a `steam_api.dll` that does not know it). The entries the Spacewar
generation needed were recovered from an SDK that is no longer on this machine, so that
part of the record is only in the copy whoever made it still holds - keep it if you need
it. What a hand edit has to keep to is still what follows, because the generator refuses
...
| `tools/steamworks_sdk_import.py` | Reads an installed Steamworks SDK's headers and writes the interface layouts - `--diff` against the ones in use, `--list`, `--selftest` with no SDK. Needs `tools/requirements.txt` (cxxheaderparser and pcpp) and nothing else here does. |
